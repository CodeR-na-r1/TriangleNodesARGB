...
void loop() {
  if (!flag) {
    ...
    delay(500);
    test1();
    Serial.println("Test1 (sendCommit on 24 adress) completed");

    getData();
    Serial.println("get data from 24 adress");
...
void test1() {
  String s1 = "TEST_4_ADDRESS_24_WITH_COMMIT";
  for (int i = 0; i < s1.length(); ++i) {
    buffer[i] = s1[i];
  }
  bus.send(24, buffer, s1.length(), true);
}
