...
SerialBusSlave bus(4, 0, 16, 13, 50);  // RX 2 TX 3 Fake 0
...
bool flag = false;
char s[15] = { '1', '3', '-', 's', 'u', 'p', 'e', 'r', 'S', 't', 'r', 'i', 'n', 'g', '!' };

void loop() {
  if (!flag) {

    getData();  // data from test 2

    Serial.println("getData 1 completed");

    bus.send(1, s, 15);  // send for getData address == 13
